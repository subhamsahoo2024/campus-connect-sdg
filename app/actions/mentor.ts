 ...
 */
export async function updateMentorshipStatus(matchId: string, status: MentorshipStatus) {
  const supabase = await createClient()
  const {
    data: { user },
  } = await supabase.auth.getUser()
...
export async function updateMeetingStatus(meetingId: string, status: MeetingStatus, notes?: string) {
  ...
  const updateData: { status: MeetingStatus; notes?: string; updated_at: string } = {
    status,
