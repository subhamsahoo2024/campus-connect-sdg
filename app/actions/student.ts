...
export async function updateStartupStage(startupId: string, newStage: StartupStage) {
...
export async function upsertStartup(formData: FormData) {
  ...
  if (startupId) {
    ...
    // First startup = 50 XP
    await supabase.rpc('increment_innovation_score', { p_user_id: user.id, p_delta: 50 })
  }
...
export async function completeMission(missionId: string, xpReward: number) {
  ...
  await supabase.rpc('increment_innovation_score', { p_user_id: user.id, p_delta: xpReward })
